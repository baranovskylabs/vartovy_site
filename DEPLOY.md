...
### Guardrails для стабільної доставки (не видаляти)

Щоб форма не ламалась повторно, тримайте такі інваріанти:

1. Єдиний API endpoint форми: `https://vartovy.app/api/contact`.
2. Одержувач у функції зафіксований: `vartovy.support@protonmail.com`.
3. Не повертати honeypot-поле `_honey` у форму й не перевіряти його у функції.
4. На клієнті залишати `_submission_id` (idempotency key) для захисту від дубльованих листів.
5. Лічильник rate-limit у KV інкрементувати тільки після успішної доставки (`res.ok`).
6. Після успіху форма має:
    - показати зелений статус,
    - розблокувати кнопку,
    - зробити redirect на `../index.html` приблизно через 2 секунди.

Мінімальна перевірка перед пушем:

- `node --check assets/js/main.js`
- `node --check functions/api/contact.js`
- Переконатися, що `pages/contact.html` підключає актуальну версію `main.js?v=...`
- Локально відправити 1 тестове повідомлення і перевірити:
   - HTTP `200` від `/api/contact`
   - зелений статус
   - redirect на головну
   - лист прийшов на `vartovy.support@protonmail.com`
